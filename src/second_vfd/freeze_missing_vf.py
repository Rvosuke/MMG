"""
Freeze-Missing-VF网络结构和处理逻辑

1. 网络结构
   - 主干网络: 使用三个SeNet作为主干网络,用于提取输入图像的特征。
   - 注意力卷积层: 在主干网络之后使用ResidualAttention层,用于捕获通道之间的依赖关系。
...
2. 处理逻辑
   ...
   - 注意力卷积: 将提取到的图像特征传递给ResidualAttention层,捕获通道之间的依赖关系。
   - 图像特征处理: 对ResidualAttention层的输出进行平均池化,并使用全连接层进行特征变换。
...
3. 设计思想
   ...
   - 注意力机制: 通过ResidualAttention层捕获通道之间的依赖关系,提高特征表示能力。
   - 残差连接: 在ResidualAttentionBlock中使用残差连接,缓解梯度消失问题,加深网络深度。
   - 模块化设计: 将网络划分为多个子模块,如ResidualAttentionBlock和make_layer,提高代码的可读性和可维护性。

4. 代码结构
   - ResidualAttentionBlock类: 实现残差块,包含左侧分支和可选的右侧shortcut连接。
   - FreezeMissingVF类: 实现整个深度学习模型,包括主干网络、注意力卷积层、全连接层等。
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        ResidualAttentionBlock的前向传播函数

        Args:
            x: 输入张量

        Returns:
            ResidualAttentionBlock的输出
        """
        out = self.left(x)
        attention = self.attend(out)
        out *= attention
...
class FreezeMissingVF(nn.Module):
    def __init__(self, freeze_bn: bool = False):
        ...
        """
        super(FreezeMissingVF, self).__init__()
        self.backbone = nn.ModuleList([SENet(2, [3, 4, 6, 3]) for _ in range(3)])
        self.freeze_bn = freeze_bn
        self.fuse = make_layer(768, 2048, 4)
    ...
    def make_layer(in_channel: int, out_channel: int, block_num: int, stride: int = 1) -> nn.Sequential:
        """
        创建layer层，包含多个ResidualAttentionBlock

        Args:
            in_channel: 输入通道数
            out_channel: 输出通道数
            block_num: ResidualAttentionBlock的个数
            stride: 第一个ResidualAttentionBlock的卷积步长，默认为1

        Returns:
            包含多个ResidualAttentionBlock的Sequential
        """
        shortcut = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, 1, stride, bias=False),
            nn.BatchNorm2d(out_channel)
        )
        layers = [ResidualAttentionBlock(in_channel, out_channel, stride, shortcut)]
        layers.extend([ResidualAttentionBlock(out_channel, out_channel) for _ in range(1, block_num)])
