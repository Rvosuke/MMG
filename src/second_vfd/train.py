...
class Trainer(object):
    def __init__(self, args):
        ...
        model = torch.load('./model_save.pth', map_location=device)
        model.eval()
        self.model = model
    ...
    def validation(self):
        ...
        for sample in tbar:
            ...
            if self.args.cuda: image, clinical = image.cuda(), clinical.cuda()
            with torch.no_grad():
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='shufa_4',
                        choices=['shufa_4'],
    ...
    if args.cuda: args.gpu_ids = [int(s) for s in args.gpu_ids.split(',')]
